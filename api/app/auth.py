...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    if user is None:
        raise credentials_exception
    # Return a dictionary that matches the User schema
    return {
        "id": user.id,
...
def authenticate_user(db: Session, username: str = None, email: str = None, password: str = None):
    try:
        ...
        if username and email:
            user = query.filter(
                (models.Usuario.usuario == username) | 
                (models.Usuario.correo == email)
            ).first()
        elif username:
            user = query.filter(models.Usuario.usuario == username).first()
        else:  # email
            user = query.filter(models.Usuario.correo == email).first()
